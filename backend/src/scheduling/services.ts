...
async function isProfessionalAvailable(professionalId: number, placeId: number, startTime: Date, endTime: Date): Promise<boolean> {
...
export async function create(newScheduling : Scheduling) {
  ...
  if(!(await isProfessionalAvailable(newScheduling.professionalId, newScheduling.placeId, newScheduling.startTime, newScheduling.endTime))) {
    throw new Error("O profissional escolhido não está disponível para esse horário, ou lugar")
  }
