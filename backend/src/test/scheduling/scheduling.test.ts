...
describe('scheduling', async () => {
  ...
  test('it should not create a new scheduling for the same place with interposed times', async () => {
    ...
    const professional : Professional = await createProfessionalForTenant(tenant.id)
    const professional2 : Professional = await createProfessionalForTenant(tenant.id)
    ...
    expect(response.statusCode).toBe(409)
    expect(response.json().message).toBe("Já existe um agendamento marcado para este horário com neste local")  
  ...
  test('it should not create a scheduling when a professional is not available for a place', async () => {
    ...
    expect(response.statusCode).toBe(409)
    expect(response.json().message).toBe("O profissional escolhido não está disponível para esse horário, ou lugar")  
  })
